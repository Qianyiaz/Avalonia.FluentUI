...
public partial class WifiPageViewModel : ObservableObject
{
    ...
    public async Task Change(IList selectedItems)
    {
        var items = selectedItems.Cast<ConnectionItem>().ToList();
    ...
    public void Remove(IList selectedItems)
    {
        foreach (var item in selectedItems.Cast<ConnectionItem>().ToList())
            Items.Remove(item);
        if (Items.Count == 0)
            IsEnabled = false;
    }
